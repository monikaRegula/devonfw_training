...
public class UcFindOrderImpl extends AbstractOrderUc implements UcFindOrder {
  ...
  @Override
  public Set<OrderEto> findOrdersByGivenDayAndStatus(@Nonnull LocalDate date, @Nonnull OrderStatus status) {

    Set<OrderEto> orderList = getBeanMapper().mapSet(getOrderRepository().findAllByCreationDateAndStatus(date, status),
        OrderEto.class);
